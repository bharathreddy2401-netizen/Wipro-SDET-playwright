...
const rawFlights = [
 ...
 "Dubai-Mumbai:450",
 "Berlin-Rome:95"];
function Firstpro(rawFlights)
{
let arr=[];
for(let i=0;i<rawFlights.length;i++)
{
    let firIndex=rawFlights[i].indexOf("-");
    let secIndex=rawFlights[i].indexOf(":");
    let from=rawFlights[i].substring(0,firIndex);
    let to=rawFlights[i].substring(firIndex+1,secIndex);
    let price=Number(rawFlights[i].substring(secIndex+1,));
...
console.log(Firstpro(rawFlights));*/
//=================================================================================================================================================================================================================================================//
...
{
    if(cart.category==="Electronics" && cart.price>200)
    {
        return true
    }
    return false;
...
function applyPromo(cart, promoCallback)
{
    ...
    {
        if(promoCallback(cart[i]))
        {
            let saving=cart[i].price*0.10;
            cart[i].price-=saving;
            savings+=saving;
            cart[i].isDiscounted=true;
            discCount++;
        }
        cart[i].isDiscounted=false;
    }
    console.log(`Promotion applied! ${discCount} items were discounted for a total saving of ${savings}.`)

    setTimeout(() => {
    console.log("Server processing complete.", cart);
    return cart;
  }, 1000);
}
...
console.log(applyPromo(cart,isEligible));*/
//=====================================================================================================================
...
function movies(movieData) {
  ...
  for (let movieString of movieData) {
    
    const parts = movieString.split("|");
    const name = parts[0];
    const genre = parts[1];
    const rawViews = parts[2];
    ...
    const isRightGenre = (genre === "Action" || genre === "Sci-Fi");
    const hasEnoughViews = (views > 5000);


    if (isRightGenre && hasEnoughViews) {
      filterMovies.push(movieObject);
    }
    //let filterMovies=movieObject.filter(d=>(d.genre==="Action" || d.genre==="Sci-Fi")&&(d.views)>5000 );
  }
    filterMovies.sort((a, b) => {
    return b.views - a.views;
  });

    return JSON.stringify(filterMovies);
}
console.log(movies(rawMovies));*/
...
function calculatePayroll(employees, taxCallback)
{
    ...
    for(let i=0;i<employees.length;i++)
    {
        ...
        let netSalary=employees[i].salary-employees[i].salary*taxCallback(employees[i].salary);
        if(netSalary>4000)
        {
            employees.status="Premium";
        }
        else{
            employees.status="Standard";
        }

        totalNetPayout += netSalary;
    }
    console.log(`Payroll Processed: Total Net Payout is ${totalNetPayout} for ${employeeCount} employees.`);

    setTimeout(() => {
    console.log("Returning processed payroll...", employees);
    return employees;
  }, 2000);
}
